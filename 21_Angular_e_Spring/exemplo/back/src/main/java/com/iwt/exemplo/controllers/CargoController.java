...
public class CargoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Cargo> acharCargo(@PathVariable("id") Long id) {
    ...
    public ResponseEntity<Cargo> registrarCargo(@RequestBody Cargo cargo) {
    ...
    public ResponseEntity<Cargo> atualizarCargo(@PathVariable("id") Long id, @RequestBody Cargo cargoAtualizado) {
        Cargo cargo = cargoRepository.findById(id).orElseThrow(
                () -> new NoSuchElementException("Cargo não encontrado para id " + id)
        );
    ...
    public ResponseEntity<?> excluirCargo(@PathVariable("id") Long id) {
        Cargo cargo = cargoRepository.findById(id).orElseThrow(
                () -> new NoSuchElementException("Cargo não encontrado para id " + id)
        );
